...
export function CanvasArea({
  ...
  const handleSlideClick = useCallback(
    (e: React.MouseEvent) => {
      // Concept markers open a question instead of toggling playback. This is
      // checked before anything else because the outer entity handler runs on
      // bubble, i.e. after this one, so it cannot prevent playback itself.
    ...
    <div className="w-full h-full flex flex-col bg-gray-50 dark:bg-gray-900 group/canvas">
      ...
      {!hideToolbar && (
        <CanvasToolbar
          ...
          onSkipDiscussionTurn={onSkipDiscussionTurn}
        />
      )}
    </div>
  );
