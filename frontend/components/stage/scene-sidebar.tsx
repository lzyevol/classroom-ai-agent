...
export function SceneSidebar({
  ...
  const isLessonClassroom = stage?.id.startsWith('lesson-') ?? false;
  const exitPath = isLessonClassroom ? '/lesson' : '/';
  const exitTitle = isLessonClassroom
    ? locale === 'zh-CN'
      ? '返回课程目录'
      ...
      <div className={cn('flex flex-col w-full h-full overflow-hidden', collapsed && 'hidden')}>
        ...
        <div className="h-10 flex items-center justify-between shrink-0 relative mt-3 mb-1 px-3">
          <button
            ...
            title={exitTitle}
          >
            <div className="flex items-center gap-2" aria-label="具身智教">
              <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 shadow-sm">
                <BookOpen className="h-4 w-4 text-white" />
              </span>
              <span className="whitespace-nowrap text-lg font-bold tracking-tight text-slate-700 dark:text-slate-100">
                具身智教
              </span>
            </div>
          </button>
