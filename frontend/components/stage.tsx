...
export function Stage({
  ...
  // TTS selection is a runtime setting. If speech is active, replay the
  // current sentence with the new provider/voice and then continue normally.
  const ttsProviderId = useSettingsStore((s) => s.ttsProviderId);
  const ttsVoice = useSettingsStore((s) => s.ttsVoice);
  const ttsSpeed = useSettingsStore((s) => s.ttsSpeed);
  const ttsEnabled = useSettingsStore((s) => s.ttsEnabled);
  const activeTTSProviderConfig = useSettingsStore(
    (s) => s.ttsProvidersConfig[s.ttsProviderId],
  );
  useEffect(() => {
    void audioPlayerRef.current.restartCurrentSpeech(engineRef.current?.getMode() === 'paused');
  }, [
    ttsProviderId,
    ttsVoice,
    ttsSpeed,
  ...
  /**
   * Route a question into the chat/QA flow.
   *
   * Shared by the roundtable input and by clicks on `.entity` concept markers
   * in the slide, so both paths pause playback and update state identically.
   */
  const askQuestionInClass = (question: string) => {
    ...
    // During playback, handleUserInterrupt saves the resume position and its
    // onUserInterrupt callback forwards the text, so don't send it twice.
    if (
      engineRef.current &&
      (engineMode === 'playing' || engineMode === 'live' || engineMode === 'paused')
    ) {
      engineRef.current.handleUserInterrupt(question);
    } else {
      chatAreaRef.current?.sendMessage(question);
    }
    chatAreaRef.current?.switchToTab('chat');
    setIsCueUser(false);
    // Mark streaming immediately so the stop button stays in sync.
    setChatIsStreaming(true);
    setChatSessionType(chatSessionType || 'qa');
    // Optimistic thinking dots, overwritten by the real SSE event.
    setThinkingState({ stage: 'director' });
  };
  ...
  /**
   * Make injected `.entity` spans reachable by keyboard and screen readers.
   *
   * The spans arrive as raw HTML from the slide renderer, so the attributes
   * have to be applied to the live DOM after each scene render.
   */
  useEffect(() => {
    const root = canvasRef.current;
    if (!root) return;
    root.querySelectorAll<HTMLElement>('.entity').forEach((marker) => {
      if (marker.closest('.pointer-events-none')) return;
      if (marker.getAttribute('role') === 'button') return;
      marker.setAttribute('role', 'button');
      marker.setAttribute('tabindex', '0');
      const label = marker.dataset.q?.trim() || marker.textContent?.trim() || '';
      if (label) marker.setAttribute('aria-label', `提问：${label}`);
    });
  }, [currentSceneId, currentScene]);

  /**
   * Delegate activation of generated `.entity` spans to the QA flow.
   *
   * Event delegation is required because the bullets are injected as raw HTML
   * by the slide renderer, so no React handler can be attached per span.
   */
  const handleCanvasClick = (event: React.MouseEvent<HTMLDivElement>) => {
    const marker = (event.target as HTMLElement).closest<HTMLElement>('.entity');
    if (!marker) return;
    // Thumbnails render inside a pointer-events-none wrapper; ignore those.
    if (marker.closest('.pointer-events-none')) return;

    const question = marker.dataset.q?.trim() || marker.textContent?.trim();
    if (!question) return;

    event.preventDefault();
    event.stopPropagation();
    askQuestionInClass(question);
  };

  const handleCanvasKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key !== 'Enter' && event.key !== ' ') return;
    const marker = (event.target as HTMLElement).closest<HTMLElement>('.entity');
    if (!marker || marker.closest('.pointer-events-none')) return;
  ...
  return (
    <div className="flex-1 flex overflow-hidden bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
        ...
        {/* Canvas Area — click delegation powers `.entity` concept questions */}
        <div
          ref={canvasRef}
        ...
        >
          <CanvasArea
            ...
            showStopDiscussion={
              engineMode === 'live' ||
              // Not gated on chatIsStreaming: a turn being held for reading has
              // finished streaming, and that is exactly when skipping matters.
              chatSessionType === 'qa' ||
              chatSessionType === 'discussion'
            }
            onStopDiscussion={handleStopDiscussion}
            onSkipDiscussionTurn={() => {
              chatAreaRef.current?.skipDiscussionTurn?.();
            }}
        ...
        {mode === 'playback' && (
          <Roundtable
            ...
            onMessageSend={askQuestionInClass}
            onDiscussionStart={() => {
              // Clear any existing whiteboard content before a new discussion,
              // otherwise new drawings overlap what earlier stages left behind.
              const stageAPI = createStageAPI(useStageStore);
              const boards = useStageStore.getState().stage?.whiteboard ?? [];
              boards.forEach((wb) => {
                if (wb.elements.length > 0) {
                  stageAPI.whiteboard.delete(wb.id);
                }
              });
