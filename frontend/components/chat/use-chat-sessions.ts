...
export function useChatSessions(options: UseChatSessionsOptions = {}) {
  ...
  const discussionAudioRef = useRef<AudioPlayer | null>(null);
  const discussionSpeechQueueRef = useRef<
    Array<{ text: string; agentId?: string; resolve: () => void }>
  >([]);
  const messageAgentIdsRef = useRef<Map<string, string>>(new Map());
  const discussionSpeechRunningRef = useRef(false);
  const discussionSpeechGenerationRef = useRef(0);
  const activeSpeechResolveRef = useRef<(() => void) | null>(null);
  /** Advances the current discussion turn early; set while a turn is held. */
  const skipDiscussionTurnRef = useRef<(() => void) | null>(null);
  const ttsProviderId = useSettingsStore((state) => state.ttsProviderId);
  const ttsVoice = useSettingsStore((state) => state.ttsVoice);
  const ttsSpeed = useSettingsStore((state) => state.ttsSpeed);
  const ttsEnabled = useSettingsStore((state) => state.ttsEnabled);
  const activeTTSProviderConfig = useSettingsStore(
    (state) => state.ttsProvidersConfig[state.ttsProviderId],
  );

  /**
   * How long to leave one discussion turn on screen.
   *
   * Roughly 6 Chinese characters per second, clamped so a one-liner still lands
   * and a long turn does not stall the conversation.
   */
  const readingTimeMs = (text: string): number => {
    const chars = text.replace(/\s+/g, '').length;
    return Math.min(Math.max((chars / 6) * 1000, 1800), 9000);
  };

  const holdDiscussionTurn = useCallback((text: string, advance: () => void) => {
    // A turn queued while another is held means the previous timer already
    // fired or was skipped; clearing keeps only one pending advance.
    skipDiscussionTurnRef.current = null;
    ...
    const timer = setTimeout(finish, readingTimeMs(text));
    skipDiscussionTurnRef.current = finish;
  }, []);

  const stopDiscussionSpeech = useCallback(() => {
    discussionSpeechGenerationRef.current += 1;
    const queuedItems = discussionSpeechQueueRef.current;
    discussionSpeechQueueRef.current = [];
    queuedItems.forEach((item) => item.resolve());
    activeSpeechResolveRef.current?.();
    activeSpeechResolveRef.current = null;
    discussionAudioRef.current?.destroy();
    discussionAudioRef.current = null;
    messageAgentIdsRef.current.clear();
    discussionSpeechRunningRef.current = false;
    // The held turn belongs to the session being torn down.
    skipDiscussionTurnRef.current = null;
  }, []);

  const drainDiscussionSpeech = useCallback(async () => {
    if (discussionSpeechRunningRef.current) return;
    discussionSpeechRunningRef.current = true;
    const generation = discussionSpeechGenerationRef.current;
    const player = discussionAudioRef.current ?? createAudioPlayer();
    discussionAudioRef.current = player;

    try {
      while (
        discussionSpeechQueueRef.current.length > 0 &&
        generation === discussionSpeechGenerationRef.current
      ) {
        const item = discussionSpeechQueueRef.current.shift();
        ...
        await new Promise<void>((resolve) => {
          activeSpeechResolveRef.current = resolve;
          player.onEnded(() => {
            activeSpeechResolveRef.current = null;
            resolve();
          });
          player
            .playText(text, { agentId: item.agentId })
            .then((started) => {
              if (!started) {
                activeSpeechResolveRef.current = null;
                resolve();
      ...
      if (generation === discussionSpeechGenerationRef.current) {
        discussionSpeechRunningRef.current = false;
      }
    }
  }, []);

  const enqueueDiscussionSpeech = useCallback(
    (text: string, agentId?: string): Promise<void> => {
      const settings = useSettingsStore.getState();
      // discussionTtsEnabled is checked separately so a student can keep the
      // lecture spoken while reading discussions silently.
      if (
        !settings.ttsEnabled ||
        !settings.discussionTtsEnabled ||
        !text.trim()
      ) {
      ...
      return new Promise<void>((resolve) => {
        discussionSpeechQueueRef.current.push({ text, agentId, resolve });
        void drainDiscussionSpeech();
      });
    },
    [drainDiscussionSpeech],
  );

  /**
   * Jump to the next discussion turn, cutting off audio that is still playing.
   *
   * Returns false when nothing is being held, so callers can leave the control
   * disabled outside a discussion.
   */
  const skipDiscussionTurn = useCallback((): boolean => {
    const advance = skipDiscussionTurnRef.current;
    if (!advance) return false;
    stopDiscussionSpeech();
    advance();
    return true;
  }, [stopDiscussionSpeech]);

  useEffect(() => () => stopDiscussionSpeech(), [stopDiscussionSpeech]);

  useEffect(() => {
    void discussionAudioRef.current?.restartCurrentSpeech();
  }, [
    ttsProviderId,
    ttsVoice,
    ttsSpeed,
    ttsEnabled,
    activeTTSProviderConfig?.apiKey,
    activeTTSProviderConfig?.baseUrl,
  ]);
  ...
  const createBufferForSession = useCallback(
      ...
      const buffer = new StreamBuffer(
        {
          onAgentStart(data: AgentStartItem) {
            const now = Date.now();
            messageAgentIdsRef.current.set(data.messageId, data.agentId);
          ...
          onAgentEnd(data) {
            messageAgentIdsRef.current.delete(data.messageId);
          ...
          onTextReveal(
            ...
            if (type !== 'lecture' && isComplete) {
              // Hold the turn just long enough to read it, then move on.
              //
              // This used to wait for the spoken audio to finish, which meant a
              // 20s clip blocked a turn a student could read in five seconds.
              // Audio now plays alongside the pacing timer instead of gating it.
              buffer.pause();
              const agentId = messageAgentIdsRef.current.get(messageId);
              void enqueueDiscussionSpeech(revealedText, agentId);
              holdDiscussionTurn(revealedText, () => {
                if (!buffer.disposed) buffer.resume();
              });
            }
          },
                ...
                prev.map((session) =>
                  session.id === sessionId
                    ? {
                        ...
                        config: {
                          ...session.config,
                          lastClassroomTrace: data.classroomTrace,
                        },
                      }
                    : session,
                ),
    ...
    [enqueueDiscussionSpeech, holdDiscussionTurn],
  );
  ...
  const endSession = useCallback(
    ...
    [activeSessionId, stopDiscussionSpeech],
  );
  ...
  const sendMessage = useCallback(
    async (content: string): Promise<void> => {
      ...
      // Fast classroom mode always uses the fixed teacher/assistant pair.
      const agentIds: string[] = ['default-1', 'default-2'];
  ...
  const startDiscussion = useCallback(
    async (request: DiscussionRequest): Promise<void> => {
      ...
      // Proactive classroom discussions are limited to teacher and assistant.
      const agentIds: string[] = ['default-1', 'default-2'];
      const triggerAgentId = agentIds.includes(agentId) ? agentId : 'default-1';
