...
function sanitizeDisplayedText(text: string): string {
  const objectStartPattern = /\{\s*"type"\s*:\s*"text"/g;
  const candidateStarts = [...text.matchAll(objectStartPattern)].map((match) => match.index);

  for (let index = candidateStarts.length - 1; index >= 0; index--) {
    const start = candidateStarts[index];
    if (start === undefined) continue;

    try {
      const parsed = JSON.parse(text.slice(start).trim()) as {
        type?: string;
        content?: unknown;
      };
      if (parsed.type !== 'text' || typeof parsed.content !== 'string') continue;

      const prefix = text.slice(0, start).trim();
      const content = parsed.content.trim();
      if (!prefix) return content;
      if (prefix === content || prefix.endsWith(content) || content.endsWith(prefix)) {
        return prefix.length >= content.length ? prefix : content;
      }
      return `${prefix}\n${content}`;
    } catch {
      // Not a complete leaked protocol object; render the original text.
    }
  }
      ...
      <span className="break-words">
        {parts.map((part: MessagePart, i: number) => {
          if (part.type === 'text' || part.type === 'step-start') {
            const text = part.type === 'text' ? sanitizeDisplayedText(part.text || '') : '';
            if (!text) return null;
