...
import { buildClassroomEvidencePrompt } from './classroom/fast-classroom';
...
const OrchestratorState = Annotation.Root({
  ...
  classroomTurn: Annotation<StatelessChatRequest['classroomTurn'] | null>,
  requestTurnStart: Annotation<number>,
...
async function directorNode(
  ...
  // Fast classroom flow: exactly one speaking agent for this user input.
  // After that agent returns, cue the user and end this graph invocation.
  if (state.classroomTurn) {
    if (state.turnCount > state.requestTurnStart) {
      write({
        type: 'cue_user',
        data: {
          fromAgentId: state.classroomTurn.selectedAgentId || undefined,
        },
      });
      return { shouldEnd: true };
    }

    const selectedAgentId = state.classroomTurn.selectedAgentId;
    if (!selectedAgentId || !state.availableAgentIds.includes(selectedAgentId)) {
      return { shouldEnd: true };
    }

    write({
      type: 'thinking',
      data: { stage: 'agent_loading', agentId: selectedAgentId },
    });
...
async function runAgentGeneration(
  ...
  const baseSystemPrompt = buildStructuredPrompt(
    agentConfig,
    state.storeState,
    discussionContext,
    state.whiteboardLedger,
  ...
  const systemPrompt = state.classroomTurn
    ? `${baseSystemPrompt}${buildClassroomEvidencePrompt(state.classroomTurn, agentId)}`
    : baseSystemPrompt;
  ...
  try {
    for await (const chunk of adapter.streamGenerate(lcMessages, {
      ...
      if (chunk.type === 'delta') {
        ...
        // Emit strictly in `ordered` sequence. It covers every text chunk and
        // action, including trailing partial deltas, so walking `textChunks`
        // positionally as well would re-emit content already sent.
...
export function buildInitialState(
  ...
  return {
    ...
    classroomTurn: request.classroomTurn || null,
    requestTurnStart: turnCount,
