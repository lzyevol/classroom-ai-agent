...
export interface SessionConfig {
  ...
  lastClassroomTrace?: ClassroomTrace;
}
...
export type ClassroomMode = 'single' | 'multi';
export type ClassroomIntent = 'stop' | 'acknowledgement' | 'confusion' | 'question';

export interface ClassroomKnowledgeChunk {
  chunk_id: string;
  content: string;
  quote: string;
...
export interface ClassroomCitation {
  chunk_id: string;
  quote: string;
  book?: string;
  chapter_title: string;
...
export interface ClassroomKnowledgeContext {
  query: string;
  chunks: ClassroomKnowledgeChunk[];
  citations: ClassroomCitation[];
  insufficient_evidence: boolean;
  error?: string;
}

export interface PreparedClassroomTurn {
  input: string;
  normalizedInput: string;
  intent: ClassroomIntent;
  knowledgeQuery: string | null;
  knowledge: ClassroomKnowledgeContext | null;
  selectedAgentId: 'default-1' | 'default-2' | null;
  preparedAt: number;
}
...
export interface StatelessChatRequest {
  ...
  config: {
    ...
    /** Fast classroom flow: one speaking agent per user turn. */
    classroomMode?: ClassroomMode;
  ...
  /** Prepared server-side classroom turn. Clients should not construct this. */
  classroomTurn?: PreparedClassroomTurn;
