 ...
 *
 * The Python FastAPI service owns the edge-tts dependency so the Next.js
 * runtime does not need to bundle a second TTS implementation.
 */
async function generateEdgeTTS(
  ...
  const response = await fetch(`${backendUrl}/api/tts`, {
    ...
    body: JSON.stringify({
      ...
      text,
      voice: config.voice,
      rate: Math.round(((config.speed || 1.0) - 1) * 500),
    }),
  });
