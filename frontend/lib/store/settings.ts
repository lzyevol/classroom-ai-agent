...
export interface SettingsState {
  ...
  asrEnabled: boolean;
  /**
   * Read discussion turns aloud. Separate from ttsEnabled because lecture
   * narration is pre-generated while discussion audio is synthesized live, so
   * many users want the lecture spoken but the discussion silent and fast.
   */
  discussionTtsEnabled: boolean;
  ...
  setDiscussionTTSEnabled: (enabled: boolean) => void;
  ...
  persist(
    (set) => {
      ...
      return {
        ...
        setDiscussionTTSEnabled: (enabled) => set({ discussionTtsEnabled: enabled }),
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        if ((state as Record<string, unknown>).discussionTtsEnabled === undefined) {
          (state as Record<string, unknown>).discussionTtsEnabled = true;
        }
